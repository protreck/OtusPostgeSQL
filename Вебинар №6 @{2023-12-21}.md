...
[pspg. Часть 1](https://ptolmachev.ru/pspg-chast-1.html)
...
Первое подключение к базе с пользователем по умолчанию:
```
...
Посмотреть конфигурационные файлы:
```
# show hba_file;
# show config_file;
```
...
Включение listener в postgresql.conf (/etc/postgresql/15/main/postgresql.conf):
```
listen_addresses='*';
```

Показать настройки listener:
```
# show listen_addresses;
```

Установить настройку listener:
```
# alter system set listen_addresses='*';
```

Включение входа по паролю в pg_hba.conf (/etc/postgresql/15/main/pg_hba.conf):
```
host all all 0.0.0.0/0 md5 (с 14 версии scram-sha-256)
```
...
\dt *s* - список всех таблдиц, содержащих s в имени
\dS - систменые таблицы
\d+ - описание таблицы
\d "table name" - описание таблицы
\i - запуск команды из внешнего файла, например, \i /my/directory/my.sql
\pset - команда настройки параметров форматирования
\echo - выводит сообщение
\set - устанавливает значение переменной среды. Без параметров выводит список текущих значений переменных. \unset - удаляет.
```

Настройка параметров psql.
```
\set ECHO HIDDEN on/off - включение отключение показа скрытых запросов.
```

**TOAST** таблицы создаются "под капотом" для обычных таблиц, данные строк которых не помещаются на одну страницу. Принадлежат схеме pg_toast. Имеют собственные индексы. Участвуют в запросах при необходимости, когда запрашиваются эти самые "длинные" данные. Само собой, попадут в запрос при использовании звездочки в SELECT.
